...
public final class DetectedServer
{
    ...
    public boolean maxPollCountExceeded()
    {
        ...
        {
			LOGGER.warn("Maximum poll count of \"" + getDisplayName() + "\" exceeded! Count = " + Integer.toString(pollCount));
            return true;
        }
        else
            return false;
    ...
    {
        ++pollCount;
		LOGGER.info("Incrementing poll count of \"" + getDisplayName() + "\" to " + Integer.toString(pollCount));
...
//    }
    ...
    {
		// LOGGER.info("Updating status of server " + getName() + " to " + status.name());
    ...
    public boolean connect()
    {
		...
		LOGGER.info("Connecting " + name.get());
		LOGGER.debug("Status = " + serverStatus.get());
    ...
    public boolean whoAreYou()
    {
        ...
        try
        {
            ...
            if (responseCode == 200)
            {
                ...
                {
					LOGGER.warn("Got an indecipherable response from " + address.getHostAddress());
                }
            }
            else if (responseCode == 503)
            {
                ...
                {
					LOGGER.warn("503 response from @ " + address.getHostAddress());
                    disconnect();
                }
            }
            else
            {
				LOGGER.warn("No response to \"" + url + "\" from @" + address.getHostAddress());
                //disconnect();
            }
        } catch (java.net.SocketTimeoutException stex)
        {
            long t2 = System.currentTimeMillis();
			LOGGER.warn("Timeout whilst asking who are you @ " + address.getHostAddress() + " - time taken = " + Long.toString(t2 - t1));
            //disconnect();
        }
        catch (IOException ex)
        {
			LOGGER.error("Error whilst asking who are you @ " + address.getHostAddress(), ex);
            disconnect();
        }
        return gotAResponse;
    }
    ...
    public List<DetectedDevice> listAttachedPrinters()
    {
        ...
        {
            long t2 = System.currentTimeMillis();
			LOGGER.error("Timeout whilst polling for remote printers @ " + address.getHostAddress() + " - time taken = " + Long.toString(t2 - t1));
        ...
        {
            disconnect();
			LOGGER.warn("Error whilst polling for remote printers @ " + address.getHostAddress(), ex);
        }
    ...
    public List<CameraInfo> listAttachedCameras()
    {
        ...
        {
            long t2 = System.currentTimeMillis();
			LOGGER.error("Timeout whilst polling for remote cameras @" + address.getHostAddress() + " - time taken = " + Long.toString(t2 - t1));
            // On a timeout, use last know list of attached cameras, to avoid flickering of camera panels.
            detectedCameras = attachedCameras;
        }
        catch (IOException ex)
        {
			LOGGER.warn("Error whilst polling for remote cameras @" + address.getHostAddress(), ex);
        }
    ...
    public Image takeCameraSnapshot(CameraSettings settings)
    {
        ...
        try
        {
            ...
            if (responseCode == 200)
            {
                ...
                if (snapshotImage.isError()) {
                    snapshotImage = null;
					LOGGER.error("Error loading image.from \"" + url + "\"@" + address.getHostAddress() + "\r\n" + snapshotImage.exceptionProperty().get().getMessage(), snapshotImage.exceptionProperty().get());
                }
            } else
            {
				LOGGER.warn("No response to \"" + url + "\"@" + address.getHostAddress());
            }
        } catch (java.net.SocketTimeoutException ex)
        {
            long t2 = System.currentTimeMillis();
        ...
        {
			LOGGER.error("Error whilst polling for remote cameras @" + address.getHostAddress(), ex);
        }

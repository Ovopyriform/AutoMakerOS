...
public class RemoteClient implements LowLevelInterface
{
    ...
    public boolean connect(String printerID) throws RoboxCommsException
    {
        ...
        {
			LOGGER.error("Failed to connect to remote printer " + remotePrinterHandle);
    ...
    public void disconnect(String printerID) throws RoboxCommsException
    {
        ...
        {
			LOGGER.error("Failed to disconnect from remote printer " + remotePrinterHandle);
    ...
    public RoboxRxPacket writeToPrinter(String printerID, RoboxTxPacket messageToWrite) throws RoboxCommsException
    {
        ...
        try
        {
			// LOGGER.info("remoteClient.writeToPrinter(" + printerID + ", " + messageToWrite.getPacketType().name());
			...
			// LOGGER.info("got response " + returnedPacket.getPacketType());
        } catch (JsonProcessingException ex)
        {
			LOGGER.warn("Didn't get correct JSON from request - passing back null for " + messageToWrite.getPacketType().name());
        } catch (IOException ex)
        {
			LOGGER.error("Failed to write to remote printer (" + messageToWrite.getPacketType().name() + ") " + remotePrinterHandle.getConnectionHandle() + " :" + ex.getMessage());
    ...
    public void clearAllErrors(String printerID) throws RoboxCommsException
    {
        ...
        {
			LOGGER.error("Failed to clear all errors on remote printer " + remotePrinterHandle, ex);
    ...
    public void clearError(String printerID, FirmwareError error) throws RoboxCommsException
    {
        ...
        {
			LOGGER.error("Failed to clear error " + error.toString() + " on remote printer " + remotePrinterHandle, ex);
    ...
    public void sendStatistics(String printerID, PrintJobStatistics printJobStatistics) throws RoboxCommsException
    {
        ...
        {
			LOGGER.error("Failed to send statistics to remote printer " + remotePrinterHandle, ex);
    ...
    public void overrideFilament(String printerID, int reelNumber, Filament filament) throws RoboxCommsException
    {
        ...
        {
			LOGGER.error("Failed to override filament on remote printer " + remotePrinterHandle.getServerPrinterIsAttachedTo().getServerIP());
	...
	public void printGCodeFile(String printerID, Path remoteFileName) throws RoboxCommsException
    {
        try
        {
			remotePrinterHandle.getServerPrinterIsAttachedTo().postRoboxPacket(baseAPIString + "/" + printerID + "/remoteControl/printGCodeFile", remoteFileName.toString(), null);
        } catch (IOException ex)
        {
            String message = "Failed to print GCode file \"" +
					remoteFileName.toString() +
                             "\" on remote printer " +
                             remotePrinterHandle.getServerPrinterIsAttachedTo().getServerIP();
			LOGGER.error(message);
            throw new RoboxCommsException(message);

...
public abstract class ReelEEPROMDataResponse extends RoboxRxPacket
{
	...
	public boolean populatePacket(byte[] byteData, float requiredFirmwareVersion)
    {
        ...
        try
        {
            ...
            {
				LOGGER.error("Couldn't parse first layer nozzle temperature - " + firstLayerNozzleTempString);
            }
            ...
            {
				LOGGER.error("Couldn't parse nozzle temperature - " + printNozzleTempString);
            }
            ...
            {
				LOGGER.error("Couldn't parse first layer bed temperature - " + firstLayerBedTempString);
            }
            ...
            {
				LOGGER.error("Couldn't parse bed temperature - " + bedTempString);
            }
            ...
            {
				LOGGER.error("Couldn't parse ambient temperature - " + ambientTempString);
            }
            ...
            {
				LOGGER.error("Couldn't parse max extrusion rate - " + filamentMultiplierString);
            }
            ...
            {
				LOGGER.error("Couldn't parse extrusion multiplier - " + feedRateMultiplierString);
            }
            ...
            {
				LOGGER.error("Failed to decode material name.");
                friendlyName = "";
            }
            ...
            {
				LOGGER.error("Couldn't parse material type from reel");
            ...
            {
				LOGGER.error("Couldn't parse remaining length - " + remainingLengthString);
            }

            success = true;
        } catch (UnsupportedEncodingException ex)
        {
			LOGGER.error("Failed to convert byte array to Printer ID Response");
        }

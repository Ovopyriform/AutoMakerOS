...
//    private final FilamentContainer filamentContainer = Lookup.getFilamentContainer();
    ...
    protected void noReelLoaded()
    {
        ...
        filamentID.set("");
        friendlyFilamentName.set(OpenAutomakerEnv.getI18N().t("smartReelProgrammer.noReelLoaded"));
    ...
    public RepairResult bringDataInBounds(Filament referenceFilamentData)
    {
        ...
        if (referenceFilamentData != null)
        {
			...
			LOGGER.debug("Reel data bounds check - result is " + result.name());
        } else
        {
			LOGGER.warn("Reel bounds check requested but reference data could not be obtained.");
        }

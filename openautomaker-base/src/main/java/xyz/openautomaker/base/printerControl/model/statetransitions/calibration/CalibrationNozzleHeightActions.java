...
public class CalibrationNozzleHeightActions extends StateTransitionActions {
	...
	public CalibrationNozzleHeightActions(Printer printer, Cancellable userCancellable, Cancellable errorCancellable, boolean safetyFeaturesRequired) {
	...
	private void clearZOffsetsOnHead() throws RoboxCommsException {
		...
		printer.transmitWriteHeadEEPROM(savedHeadData.getHeadTypeCode(), savedHeadData.getUniqueID(), savedHeadData.getMaximumTemperature(), savedHeadData.getThermistorBeta(), savedHeadData.getThermistorTCal(),
				nozzle1Data.getDefaultXOffset(), nozzle1Data.getDefaultYOffset(), 0, nozzle1Data.getMinBOffset(), savedHeadData.getFilamentID(0), savedHeadData.getFilamentID(1), nozzle2Data.getDefaultXOffset(),
				nozzle2Data.getDefaultYOffset(), 0, nozzle2Data.getMaxBOffset(), savedHeadData.getLastFilamentTemperature(0), savedHeadData.getLastFilamentTemperature(1), savedHeadData.getHeadHours());
	...
	public void doMeasureZDifferenceAction() throws PrinterException, CalibrationException {
		...
		LOGGER.debug("Nozzle height difference test");
		...
		for (int testCount = 0; testCount < numberOfNozzleHeightDifferenceTests; testCount++) {
			...
			LOGGER.debug("Delta from " + nozzleFrom + " to " + nozzleTo + " -> " + deltaValue);
			flipFlop = !flipFlop;
		}
		...
		success = true;
		LOGGER.debug("Average Z Offset was " + zDifference);
		...
		if (savedHeadData != null) {
			try {
				...
				printer.transmitWriteHeadEEPROM(savedHeadData.getHeadTypeCode(), savedHeadData.getUniqueID(), savedHeadData.getMaximumTemperature(), savedHeadData.getThermistorBeta(), savedHeadData.getThermistorTCal(),
						...
						savedHeadData.getNozzle2XOffset(), savedHeadData.getNozzle2YOffset(), savedHeadData.getNozzle2ZOffset(), savedHeadData.getNozzle2BOffset(), savedHeadData.getLastFilamentTemperature(0),
						savedHeadData.getLastFilamentTemperature(1), savedHeadData.getHeadHours());
	...
	public void saveSettings() throws RoboxCommsException {
		LOGGER.debug("zDifference is " + zDifference);
		LOGGER.debug("zco is " + zco);
		printer.transmitWriteHeadEEPROM(savedHeadData.getHeadTypeCode(), savedHeadData.getUniqueID(), savedHeadData.getMaximumTemperature(), savedHeadData.getThermistorBeta(), savedHeadData.getThermistorTCal(),
				savedHeadData.getNozzle1XOffset(), savedHeadData.getNozzle1YOffset(), (float) (-zco.get() - (0.5 * zDifference)), savedHeadData.getNozzle1BOffset(), savedHeadData.getFilamentID(0), savedHeadData.getFilamentID(1),
				savedHeadData.getNozzle2XOffset(), savedHeadData.getNozzle2YOffset(), (float) (-zco.get() + (0.5 * zDifference)), savedHeadData.getNozzle2BOffset(), savedHeadData.getLastFilamentTemperature(0),
				savedHeadData.getLastFilamentTemperature(1), savedHeadData.getHeadHours());
	}

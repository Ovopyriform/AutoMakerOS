...
public class PathHelper
{
    ...
    /**
     * Creates an internal flatten representation of path for efficient query of
     * position data along the path. path - path to be snap shot and converted.
     * flatness - the maximum allowable distance between the control points and
     * the flattened curve. return value - true is creation is successful
     * otherwise false.
     *
     * Implementation detail: The constructor is required to create an internal
     * representation of the given path with the flatness specified. The
     * internal representation needs to keep (a) the flatten path, (b) the
     * normalized length of each segment in the path, and (c) the normal of each
     * point of the segment.
     */
    public PathHelper(Path2D path,
    ...
    /**
     * Return the position data along this path at the given alpha value. alpha
     * - a value within [0, 1] that maps to the begin to end of this path.
     * requiredNormal - specifys normal information is needed. result - returns
     * the computed position if it is not null. return value - result if it is
     * not null otherwise create a new Position2D object with the computed
     * position information.
     *
     * Implementation detail: The method can do a binary search of the connected
     * segments, possibly an array, key on the normalized length of each
     * segment.
     */
    public Point2D getPosition2D(double alpha, boolean requiredNormal)
    {
        ...
        double returnX, returnY, rotation;

        double ratio = partLength / seg.length;
        Segment prevSeg = seg.prevSeg;

        returnX = prevSeg.toX + (seg.toX - prevSeg.toX) * ratio;
        returnY = prevSeg.toY + (seg.toY - prevSeg.toY) * ratio;
//        result.rotateAngle = seg.rotateAngle;
    ...
    /**
     * Interpolates angle according to rate, with correct 0->360 and 360->0
     * transitions
     */
    ...
    /**
     * Returns the index of the first segment having accumulated length from the
     * path beginning, greater than {@code length}
     */
    ...
    private static class Segment
    {
        ...
        public static Segment newLineTo(Segment fromSeg, double toX, double toY)
        {
            ...
            { // filtering out flattening noise

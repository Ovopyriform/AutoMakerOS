...
public class AMFOutputConverter implements MeshFileOutputConverter {
	...
	void outputMaterial(int materialId, float r, float g, float b, XMLStreamWriter streamWriter) throws XMLStreamException {
		outputMaterial(2, 0.1f, 0.1f, 0.1f, Optional.empty(), streamWriter);
	}

	void outputMaterial(int materialId, float r, float g, float b, Optional<Float> a, XMLStreamWriter streamWriter) throws XMLStreamException {
	...
	private void outputVertex(MeshToWorldTransformer meshToWorldTransformer, MeshView meshView, ObservableFloatArray points, int offset, XMLStreamWriter streamWriter) throws XMLStreamException {
		Point3D transformedVertex = meshToWorldTransformer.transformMeshToRealWorldCoordinates(points.get(offset), points.get(offset + 1), points.get(offset + 2));
	...
	@Override
	public MeshExportResult outputFile(List<MeshForProcessing> meshesForProcessing, String printJobUUID, Path printJobDirectory, boolean outputAsSingleFile) {
		List<String> createdFiles = new ArrayList<>();
		List<Vector3D> centroids = new ArrayList<>();

		Path tempModelFilenameWithPath = printJobDirectory.resolve(printJobUUID + BaseConfiguration.amfTempFileExtension);

		createdFiles.add(tempModelFilenameWithPath.toString());

		try {
			FileWriter fileWriter = new FileWriter(tempModelFilenameWithPath.toFile());
	...
	/**
	 * This could be much simpler if it just wrapped the interface, but it was free off the internet so I can't argue... It pretty-prints the XML.
	 */

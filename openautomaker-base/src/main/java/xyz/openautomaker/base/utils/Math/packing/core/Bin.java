...
import xyz.openautomaker.base.utils.Math.packing.primitives.MPointDouble;
import xyz.openautomaker.base.utils.Math.packing.primitives.MVector;
import xyz.openautomaker.base.utils.Math.packing.utils.Utils;
...
public class Bin
{
    ...
    public Bin(Dimension dimension, MArea[] partsThatShouldNotBePlaced)
    {
        this(dimension);

        java.util.List<MArea> newlyPlacedParts = new ArrayList<>();

        for (MArea partNotToBePlaced : partsThatShouldNotBePlaced)
        {
            newlyPlacedParts.add(partNotToBePlaced);

            Rectangle2D.Double addedBounds = partNotToBePlaced.getBoundingBox2D();
            computeFreeRectangles(addedBounds, freeRectangles);
        }
        this.placedPieces = newlyPlacedParts.toArray(new MArea[0]);
     ...
     *
     * @param usedFreeArea rectangular area that contains the newly placed
     * piece.
     ...
     * @return <ul>
     * <li><b>true</b> if some piece changed its position, that is, if was
     * placed inside other.</li>
     ...
     * @param collisionArea MArea that contains positions to be avoided when
     * moving the piece.

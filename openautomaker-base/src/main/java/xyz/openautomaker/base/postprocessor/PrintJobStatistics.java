...
public class PrintJobStatistics {
	...
	public void updateValueFromStatsString(String statsString) {
		...
		if (statsString.startsWith(DATA_PREFIX_IN_FILE)
			...
			try {
				...
				LOGGER.error("Couldn't update field " + fieldName);
			}
		}
	}

	public void writeStatisticsToFile(Path statisticsFilePath) throws IOException {

		File statsFile = statisticsFilePath.toFile();

		FileUtils.ensureParentDir(statsFile);
			...
			for (Field field : this.getClass().getDeclaredFields()) {
				if (!(field.getDeclaredAnnotations().length == 1
					...
					try {
						...
						LOGGER.error("Exception processing " + field.getName(), ex);
					}
				}
			}
			writer.write(";#########################################################\n");
	...
	public static PrintJobStatistics importStatisticsFromGCodeFile(Path roboxisedFileLocation) throws FileNotFoundException, IOException {
		PrintJobStatistics result = new PrintJobStatistics();

		//TODO: try with resources
		File roboxisedFile = roboxisedFileLocation.toFile();

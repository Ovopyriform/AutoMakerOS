...
import xyz.openautomaker.base.configuration.slicer.NozzleParameters;
import xyz.openautomaker.base.postprocessor.CannotCloseFromPerimeterException;
import xyz.openautomaker.base.postprocessor.NoPerimeterToCloseOverException;
import xyz.openautomaker.base.postprocessor.NotEnoughAvailableExtrusionException;
import xyz.openautomaker.base.postprocessor.NozzleProxy;
import xyz.openautomaker.base.postprocessor.PostProcessingError;
import xyz.openautomaker.base.postprocessor.nouveau.NodeManagementUtilities.AvailableExtrusion;
...
import xyz.openautomaker.base.postprocessor.nouveau.nodes.NodeProcessingException;
...
import xyz.openautomaker.base.postprocessor.nouveau.nodes.nodeFunctions.IteratorWithOrigin;
                ...
                && !(sectionContainingNodeToAppendClosesTo instanceof InnerPerimeterSectionNode))
        {
            ...
            if (!processedOK)
            {
                ...
                if (result.isPresent())
                {
                    ...
                    {
//                        LOGGER.error("Failed to copy close from retract in non-perimeter - outer excluded");
                    }
                }
            }
//
...
//                {
////                    LOGGER.error("Failed to copy close from retract in non-perimeter");
//                }
//            }
        ...
        } else if (sectionContainingNodeToAppendClosesTo instanceof InnerPerimeterSectionNode)
        {
            ...
            {
//                LOGGER.error("Failed to close from retract in inner perimeter");
            }
        } else
        {
            boolean processedOK = false;
            ...
            if (result.isPresent())
            {
                ...
                {
//                    LOGGER.error("Failed to copy close from retract in outer perimeter - outer excluded");
                }
            }
            ...
            if (!processedOK)
            {
                ...
                {
//                    LOGGER.error("Failed to copy close from retract in outer perimeter");
                }
            }
            ...
            NozzleProxy nozzleInUse)
    {
        ...
        {
			LOGGER.error("Not Got Enough Available Extrusion - shouldn't see this");
        }
...
//    }
     ...
     * @return CloseResult
     * @throws xyz.openautomaker.base.postprocessor.NotEnoughAvailableExtrusionException
     */
     ...
     * @return
     * @throws xyz.openautomaker.base.postprocessor.NotEnoughAvailableExtrusionException
     * @throws xyz.openautomaker.base.postprocessor.nouveau.nodes.NodeProcessingException
     */
...
//            }
        ...
        {
			LOGGER.error("No final node from copy close");
        }
        ...
        if (featureSet.isEnabled(PostProcessorFeature.OPEN_AND_CLOSE_NOZZLES))
        {
            ...
            while (layerChildIterator.hasNext())
            {
                ...
                if (layerChild instanceof RetractNode)
                {
                    ...
                    {
						LOGGER.warn("Removed retract on layer " + layerNode.getLayerNumber() + " with no prior tool selected");
            ...
            retractNodes.stream().map((retractHolder) -> {
                if (retractHolder.getNozzle() != null)
                {
                    ...
                    if (!success)
                    {
						LOGGER.warn("Close failed - removing retract anyway on layer " + layerNode.getLayerNumber());
                    }
                }
        ...
        try
        {
            ...
            } else
            {
                ...
                if (lastLayer == null)
                {
					...
					LOGGER.warn("Discarding retract from layer " + thisLayer.getLayerNumber());
                } else
                {

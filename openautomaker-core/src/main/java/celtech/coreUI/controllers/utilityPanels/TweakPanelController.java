...
public class TweakPanelController implements Initializable, StatusInsetController, PrinterListChangesListener {
	...
	private final ChangeListener<PrintQueueStatus> printQueueStatusListener = new ChangeListener<>() {
		@Override
	...
	private final ChangeListener<Number> feedRate1ChangeListener = (ObservableValue<? extends Number> observable, Number oldValue, Number newValue) -> {
	...
	private final ChangeListener<Number> feedRate2ChangeListener = (ObservableValue<? extends Number> observable, Number oldValue, Number newValue) -> {
	...
	private final ChangeListener<Number> speedMultiplier1SliderListener = (ObservableValue<? extends Number> observable, Number was, Number now) -> {
		if (!inhibitFeedrate1
			...
			try {
				...
				LOGGER.error("Error setting feed rate multiplier - " + ex.getMessage());
			}
		}
	};

	private final ChangeListener<Number> speedMultiplier2SliderListener = (ObservableValue<? extends Number> observable, Number was, Number now) -> {
		if (!inhibitFeedrate2
	...
	private final ChangeListener<Number> bedTargetTemperatureChangeListener = (ObservableValue<? extends Number> observable, Number oldValue, Number newValue) -> {
		inhibitBed = true;
	...
	private final ChangeListener<Number> bedTempSliderListener = (ObservableValue<? extends Number> observable, Number was, Number now) -> {
		if (!inhibitBed
	...
	private final ChangeListener<Number> extrusionMultiplier1ChangeListener = (ObservableValue<? extends Number> observable, Number oldValue, Number newValue) -> {
	...
	private final ChangeListener<Number> extrusionMultiplier1SliderListener = (ObservableValue<? extends Number> observable, Number was, Number now) -> {
		if (!inhibitExtrusion1
			...
			try {
				...
				LOGGER.error("Failed to set extrusion multiplier");
			}
		}
	};

	private boolean inhibitExtrusion2 = false;
	private final ChangeListener<Number> extrusionMultiplier2ChangeListener = (ObservableValue<? extends Number> observable, Number oldValue, Number newValue) -> {
	...
	private final ChangeListener<Number> extrusionMultiplier2SliderListener = (ObservableValue<? extends Number> observable, Number was, Number now) -> {
	...
	private final ChangeListener<Number> material1TargetTempChangeListener = (ObservableValue<? extends Number> observable, Number oldValue, Number newValue) -> {
	...
	private final ChangeListener<Number> lastFilamentTempChangeListener = (ObservableValue<? extends Number> observable, Number oldValue, Number newValue) -> {
		updateTemperatureSliderBounds();
	};

	private final ChangeListener<Number> material1TempSliderListener = (ObservableValue<? extends Number> observable, Number was, Number now) -> {
	...
	private final ChangeListener<Number> nozzleTargetTemp2ChangeListener = (ObservableValue<? extends Number> observable, Number oldValue, Number newValue) -> {
	...
	private final ChangeListener<Number> material2TempSliderListener = (ObservableValue<? extends Number> observable, Number was, Number now) -> {
	...
	private void bind() {
		...
		try {
			...
			LOGGER.debug("Failed to get print job statistics for tweaks page");
		}

		container.setVisible(true);

		if (currentPrinter.getPrinterAncillarySystems().bedHeaterModeProperty().get() == HeaterMode.FIRST_LAYER) {
			bedDisplay.setText(String.format("%d°C", currentPrinter.getPrinterAncillarySystems().bedFirstLayerTargetTemperatureProperty().get()));
			bedTemperatureSlider.setValue(currentPrinter.getPrinterAncillarySystems().bedFirstLayerTargetTemperatureProperty().get());
		}
		else {
			bedDisplay.setText(String.format("%d°C", currentPrinter.getPrinterAncillarySystems().bedTargetTemperatureProperty().get()));
			bedTemperatureSlider.setValue(currentPrinter.getPrinterAncillarySystems().bedTargetTemperatureProperty().get());
		}

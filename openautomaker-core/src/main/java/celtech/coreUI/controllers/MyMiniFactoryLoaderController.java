...
public class MyMiniFactoryLoaderController implements Initializable {
	...
	public void loadWebData() {
		...
		webEngine.setUserDataDirectory(env.getUserPath(TEMP).toFile());

		webContentContainer.getChildren().addAll(webView);

		if (env.getMachineType() == MachineType.MAC) {
		...
		webEngine.getLoadWorker().stateProperty().addListener((ObservableValue<? extends Worker.State> ov, Worker.State oldState, Worker.State newState) -> {
			switch (newState) {
				...
				case SUCCEEDED:
					...
					Object fileLinkFunction = webEngine.executeScript("window.autoMakerGetFileLink");
					if (fileLinkFunction instanceof JSObject) {
						fileDownloadLocation.set((String) webEngine.executeScript("window.autoMakerGetFileLink()"));
					}

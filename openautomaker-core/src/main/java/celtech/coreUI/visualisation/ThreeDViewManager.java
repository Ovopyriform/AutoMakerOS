...
public class ThreeDViewManager implements Project.ProjectChangesListener, ScreenCoordinateConverter {
	...
	private final ChangeListener<LayoutSubmode> layoutSubmodeListener = new ChangeListener<>() {
		@Override
	...
	private final ChangeListener<DragMode> dragModeListener = new ChangeListener<>() {
		@Override
	...
	private void handleMouseDoubleClickedEvent(MouseEvent event) {
		...
		if (intersectedNode instanceof MeshView) {
			if (inSelectedGroupButNotSelected.contains(intersectedNode.getParent())) {
				return;
			}
			...
			if (selectedMeshViews.contains(intersectedNode)) {
			...
			if (handleThisEvent) {
				if (intersectedNode instanceof MeshView) {
					switch (layoutSubmode.get()) {
						case SNAP_TO_GROUND:
							ModelContainer rootModelContainer = ModelContainer.getRootModelContainer((MeshView) intersectedNode);
	...
	private void doSelectTranslateModel(Node intersectedNode, Point3D pickedPoint, MouseEvent event, boolean findParentGroup) {
		...
		if (findParentGroup && !pickedModel.isSelected()) {
			while (pickedModel.getParentModelContainer() instanceof ModelContainer) {
				pickedModel = pickedModel.getParentModelContainer();
			}
		}
	...
	public ThreeDViewManager(ModelContainerProject project,
		...
		/**
		 * Set up filament, application mode and printer listeners so that the correct model colours are displayed.
		 */
	...
	/**
	 * If either the chosen filaments, x/y/z position , application mode or printer changes then this must be called. In LAYOUT mode the filament colours should reflect the project filament colours except if the position is off the bed then that
	 * overrides the project colours. In SETTINGS mode the filament colours should reflect the project print settings filament colours, taking into account the support type.
	 */
	...
	private ChangeListener<Number> cutHeightChangeListener = new ChangeListener<>() {
		@Override
	...
	public List<ModelContainer> cutModelAt(ModelContainer modelContainer, double height) {
		...
		////            LOGGER.info("Bounds of model " + modelBounds);
		////            CSG copyOfmodelAsCSG = modelAsCSG.clone();
		//            csgTimer.timerStop(this, "toCSG");
		//            LOGGER.info("Time to CSG " + csgTimer.timeTimeSoFar_ms(this, "toCSG"));
		//
		...
		////            LOGGER.info("Time to cut " + csgTimer.timeTimeSoFar_ms(this, "cut"));
		////
		...
		////            LOGGER.info("Bounds of cutbox " + boxBounds);
		//////            CSG bottomCutbox = new Cube(boxCentreBottom, boxWHD).toCSG();
		////            csgTimer.timerStop(this, "createCube");
		////            LOGGER.info("Time to createCube " + csgTimer.timeTimeSoFar_ms(this, "createCube"));
		...
		//            LOGGER.info("Time to split " + csgTimer.timeTimeSoFar_ms(this, "split"));
		//
		...
		//            LOGGER.info("Time to extract mesh " + csgTimer.timeTimeSoFar_ms(this, "toMesh"));
		//
		//            LOGGER.info("There are " + topPartMeshes.size() + " meshes");
		//
		////            csgTimer.timerStart(this, "Split");

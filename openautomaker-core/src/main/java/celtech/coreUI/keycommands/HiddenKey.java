	...
	private final EventHandler<KeyEvent> hiddenCommandEventHandler = (KeyEvent event) -> {
		...
		if (parameterCaptureInProgress) {
			if (event.getCharacter().equals("\r")) {
				LOGGER.debug("Got captured parameter " + parameterCaptureBuffer);
			...
			for (String commandSequence : commandSequences) {
				...
				else if (commandSequence.startsWith(hiddenCommandKeyBuffer + event.getCharacter())) {
			...
			if (matchedNone) {
				for (String parameterCaptureSequence : parameterCaptureSequences) {
					if (!parameterCaptureInProgress
							&& parameterCaptureSequence.equals(hiddenCommandKeyBuffer + event.getCharacter())) {
						hiddenCommandKeyBuffer += event.getCharacter();
						parameterCaptureInProgress = true;
						matchedNone = false;
						break;
					}
					else if (parameterCaptureSequence.startsWith(hiddenCommandKeyBuffer + event.getCharacter())) {
						hiddenCommandKeyBuffer += event.getCharacter();
						matchedNone = false;
						break;
					}

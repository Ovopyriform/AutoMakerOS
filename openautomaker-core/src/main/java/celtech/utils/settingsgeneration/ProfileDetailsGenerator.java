...
import xyz.openautomaker.base.configuration.profilesettings.PrintProfileSettingsTab;
...
public class ProfileDetailsGenerator {
		...
		headerSettings.forEach(setting -> {
			...
			switch (valueType) {
				case FLOAT:
					...
					break;
				default:
					LOGGER.error("Setting value type of " + valueType + " is not yet supported for 'header settings'");
			}
		});
	}
			...
			switch (valueType) {
				...
				case OPTION:
					if (printProfileSetting.getOptions().isPresent()) {
						...
						LOGGER.warn("Option setting, " + printProfileSetting.getId() + ", has no options, setting will be ignored");
					}
					break;
				case NOZZLE:
				...
				case NUMBER_LIST:
					...
					break;
				default:
					LOGGER.error("Value type of " + valueType + " not recognised, setting will be ignored");
			}
		}
	}

	/**
	 * This method gets the {@link ScrollPane} to behave in a sensible-ish manner It's height is bound to a multiple of the height of the window which means it will resize with the window and a scrollbar will appear once the content exceeds it's length.
	...
	/**
	 * Sets up the {@link ColumnConstraints} for the provided {@link GridPane} This set up is for a clean and consistent setting generation.
	 *
	...
	/**
	 * Add a single field setting to the given {@link GridPane} It consists of a {@link Label} and a {@link RestrictedNumberField}.
	 *
	...
	/**
	 * Add a combobox field setting to the given {@link GridPane} It consists of a {@link Label} and a {@link ComboBox}.
	 *
	 * @param gridPane            the pane to add to
	 * @param printProfileSetting the setting parameters
	 * @param row                 the row number of the grid to add to
	...
	/**
	 * Add a nozzle selection box and field setting to the given {@link GridPane} It consists of a {@link Label} a {@link ComboBox} for selection of a nozzle and a {@link RestrictedNumberField} for the value.
	 *
	 * @param gridPane            the pane to add to
	 * @param printProfileSetting the setting parameters
	 * @param row                 the row number of the grid to add to
	...
	/**
	 * Add a double field setting to the given {@link GridPane} for settings with a value per nozzle. It consists of a {@link Label} and two {@link RestrictedNumberField}
	 *
	 * @param gridPane            the pane to add to
	 * @param printProfileSetting the setting parameters
	 * @param row                 the row number of the grid to add to
	...
	private ComboBox setupComboBoxForNozzleSelection(PrintProfileSetting printProfileSetting, ComboBox comboBox) {
		...
		if (currentHead.getNozzleHeaters().size() == 2 && slicerType != SlicerType.Cura) {
		...
		if (slicerType != SlicerType.Cura && printProfileSetting.getId().equals("firstLayerNozzle"))
			comboBox.setDisable(true);
	...
	/**
	 * Create a {@link HBox} that contains a {@link RestrictedNumberField} with an Optional {@link Label} for the unit.
	 *
	 * @param printProfileSetting the setting parameters
	 * @return a HBox
	 */
	...
	/**
	 * Create a {@link HBox} that contains a {@link RestrictedNumberField} with an Optional {@link Label} for the unit. Also pass in the {@link ComboBox} that links to the field.
	 *
	 * @param printProfileSetting the setting parameters
	 * @return a HBox
	 */
	...
	/**
	 * A very specific method. We need to change the label and tooltip text if there are no valves, i.e. it's actually a retraction amount.
	 *

		...
		for (PolygonIndices polygonIndices : loopsOfVerticesOnOpenFace) {
			...
			if (!added) {
				...
				if (!innerLoopSets.isEmpty()) {
					...
					LOGGER.debug("new loop set " + newLoopSet.id + " contains previous top level loopsets " + ids);
...
class PolygonIndices extends ArrayList<Integer> {
	...
	private static final long serialVersionUID = 8370661490691873168L;
	/**
...
class LoopSet {
	...
	public LoopSet(CutResult cutResult, PolygonIndices outerLoop, Set<LoopSet> innerLoopSets) {
		...
		id = nextId;
		LOGGER.debug("created loopset " + id);
		nextId++;
	}
	...
	public void addToContainingChild(PolygonIndices polygonIndices) {
		...
		if (!added) {
			...
			if (!innerInnerLoopSets.isEmpty()) {
				...
				LOGGER.debug("new loop set " + newLoopSet.id + " contains previous inner loopsets " + ids + " of loopsets " + id);
		...
		if (!added) {
			...
			innerLoopSets.add(newLoopSet);
			LOGGER.debug("loopset " + id + " has new inner loop set " + newLoopSet.id);
		}
	}
}

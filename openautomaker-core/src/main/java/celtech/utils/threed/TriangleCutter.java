...
public class TriangleCutter {
	...
	static int addNewOrGetVertex(TriangleMesh mesh, Vertex intersectingVertex) {
		for (int i = 0; i < mesh.getPoints().size() / 3; i++) {
			...
			if (vertex.equals(intersectingVertex)) {
				//                LOGGER.debug("vertex already exists at " + i);
				return i;
			}
		}

		mesh.getPoints().addAll(intersectingVertex.x, intersectingVertex.y,
				intersectingVertex.z);
		//        LOGGER.debug("add new vertex at index " + (mesh.getPoints().size() / 3 - 1) + " at "
		//                + intersectingVertex);
	...
	public static void splitFaceAndAddLowerFacesToMesh(TriangleMesh mesh, Set<Integer> facesToRemove,
		...
		//        LOGGER.debug("face " + faceIndex + " TB " + topBottom);
		int v0 = mesh.getFaces().get(faceIndex * 6);
		int v1 = mesh.getFaces().get(faceIndex * 6 + 2);
		int v2 = mesh.getFaces().get(faceIndex * 6 + 4);

		//        LOGGER.debug("v0 " + getVertex(mesh, v0));
		//        LOGGER.debug("v1 " + getVertex(mesh, v1));
		//        LOGGER.debug("v2 " + getVertex(mesh, v2));
		...
		//        LOGGER.debug("v0height " + v0Height);
		//        LOGGER.debug("v1height " + v1Height);
		//        LOGGER.debug("v2height " + v2Height);
		...
		//       LOGGER.debug("b01 b12 b02 " + b01 + " " + b12 + " " + b02);
		if (!b01 && !b12 && !b02) {
		...
		if (vertexIndices.size() == 1) {
			LOGGER.debug("one vertex on plane");
	...
	private static void cutWithOneVertexOnPlane(Set<Integer> vertexIndices,
		...
		if (edgeVerticesIndices.size() != 1) {
			LOGGER.debug("vertices are ");
			v0 = mesh.getFaces().get(faceIndex * 6);
			v1 = mesh.getFaces().get(faceIndex * 6 + 2);
			v2 = mesh.getFaces().get(faceIndex * 6 + 4);
			LOGGER.debug("v0A " + getVertex(mesh, v0));
			LOGGER.debug("v1A " + getVertex(mesh, v1));
			LOGGER.debug("v2A " + getVertex(mesh, v2));
	...
	static Vertex getIntersectingVertex(Edge edge, TriangleMesh mesh, float cutHeight,
		...
		Point3D intersectingPointInBed = new Point3D(interX, cutHeight, interZ);

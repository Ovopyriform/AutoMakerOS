...
/**
 * OpenFaceCloser takes a cut mesh, which therefore has an open face) and closes the open face by triangulating the perimeters of the cut walls. It must identify which perimeters are inside which, thereby identifying holes which must be present in
 * the closing face.
 *
 * @author tony
 */
public class OpenFaceCloser {
	...
	/**
	 * Take the given mesh and vertices of the open face, close the face and add the new face to the mesh and return it.
	 */
	...
	/**
	 * We need to capture the vertex id of perimeter points so that when we get the point back after triangulation we know which point it was (after eg it was perturbed).
	 */
	...
	/**
	 * Make a Polygon for the given vertices. 3D points should be in bed coordinates so that only X and Z are required (Y being a constant at the cut height in bed coordinates).
	 */
	...
	/**
	 * For each triangle in the polygon add a face to the mesh. If any point in any triangle is not one of the outerVertices then also add that point to the mesh. Only add new vertices to the mesh if they are not one of the (already existing) vertices on
	 * the outer or inner polygons.
	 */
	...
	/**
	 * Make a Vertex for the point, in bed coordinates (so that equality comparisons are all in bed coordinates), and add the points to the mesh in local coordinates.
	 */
	...
	/**
	 * Introduce a tiny bit of noise into the XZ position of each perimeter vertex, to avoid problems in the Delauney triangulation.
	 */

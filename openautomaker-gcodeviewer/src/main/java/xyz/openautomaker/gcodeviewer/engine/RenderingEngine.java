...
import xyz.openautomaker.gcodeviewer.entities.CenterPoint;
import xyz.openautomaker.gcodeviewer.entities.Entity;
import xyz.openautomaker.gcodeviewer.entities.Floor;
import xyz.openautomaker.gcodeviewer.entities.Light;
import xyz.openautomaker.gcodeviewer.entities.PrintVolume;
import xyz.openautomaker.gcodeviewer.gui.GUIManager;
...
public class RenderingEngine {
	...
	public void start(String gCodeFile) {
		LOGGER.debug("Starting RenderingEngine.");
		...
		LOGGER.debug("Running rendering loop.");
		...
		if (fileLoader != null && fileLoader.loadFinished()) {
			try {
				if (fileLoader.loadSuccess())
				{
					...
					LOGGER.info("Number of lines = " + Integer.toString(renderParameters.getNumberOfLines()));
					...
					glfwSetWindowTitle(windowId, OpenAutoMakerEnv.getI18N().t("window.titleWithFileName").replaceAll("#1", f.getName()));
				}
			}

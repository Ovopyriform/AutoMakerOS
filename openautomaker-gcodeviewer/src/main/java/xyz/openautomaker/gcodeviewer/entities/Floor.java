package xyz.openautomaker.gcodeviewer.entities;

import xyz.openautomaker.gcodeviewer.engine.ModelLoader;
	...
	private RawModel generateFloor(ModelLoader modelLoader) {
		...
		for(int i = 0; i<VERTEX_COUNT; i++){
			for(int j = 0; j < VERTEX_COUNT; j++){
				vertices[vertexPointer * 3] = sizeX * (j / ((float) VERTEX_COUNT - 1));
				vertices[vertexPointer * 3 + 1] = sizeY * (i / ((float) VERTEX_COUNT - 1));

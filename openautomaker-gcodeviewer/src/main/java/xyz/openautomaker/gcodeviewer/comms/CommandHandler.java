...
import xyz.openautomaker.gcodeviewer.engine.RenderingEngine;
...
public class CommandHandler {
	...
	public void start() {
		...
		LOGGER.debug("Starting command handler.");
		commandQueue.start();
	}
			...
			if (command.equalsIgnoreCase("q") || command.equalsIgnoreCase("quit")) {
				...
				try {
					...
					if (commandScanner.hasNext()) {
						...
						default:
							LOGGER.error("Ignoring command " + command);
							break;
						}
					}
				}
				catch (RuntimeException ex) {
					LOGGER.error("Command parsing error", ex);
				}
			}
		}
	...
	private void processHideCommand(String command, Scanner commandScanner) {
		...
		case "t":
			if (commandScanner.hasNextInt())
				...
				LOGGER.error("Unrecognised option in command " + command);
			break;

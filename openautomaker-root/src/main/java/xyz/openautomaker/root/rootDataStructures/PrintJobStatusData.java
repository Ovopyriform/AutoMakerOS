	...
	public void updateFromPrinterData(String printerID) {
		...
		switch (printer.busyStatusProperty().get()) {
			...
			case UNLOADING_FILAMENT_E:
				printerStatusString = OpenAutoMakerEnv.getI18N().t(printer.busyStatusProperty().get().getI18nString());
		...
		if (!statusProcessed) {
			switch (printer.pauseStatusProperty().get()) {
				...
				case PAUSE_PENDING:
				case RESUME_PENDING:
					printerStatusString = OpenAutoMakerEnv.getI18N().t(printer.pauseStatusProperty().get().getI18nString());
		...
		if (!printer.getActiveErrors().isEmpty()) {
			...
			for (int errorCounter = 0; errorCounter < printer.getActiveErrors().size(); errorCounter++) {
				activeErrors[errorCounter] = OpenAutoMakerEnv.getI18N().t(printer.getActiveErrors().get(errorCounter).getErrorTitleKey());
			}
		}
	}

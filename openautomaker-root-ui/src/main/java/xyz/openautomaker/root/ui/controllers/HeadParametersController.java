...
import xyz.openautomaker.root.ui.remote.HeadEEPROMData;
...
public class HeadParametersController implements Initializable, Page {
	...
	public void updateHeadEEPROMData(HeadEEPROMData headData)
	{
		...
		headTitleBold.setText(OpenAutoMakerEnv.getI18N().t(prefix + ".titleBold"));
		headTitleLight.setText(OpenAutoMakerEnv.getI18N().t(prefix + ".titleLight"));
		headTitleEdition.setText(OpenAutoMakerEnv.getI18N().t(prefix + ".titleEdition"));
		headDescription.setText(OpenAutoMakerEnv.getI18N().t(prefix + ".description"));
		headNozzlesDescription.setText(OpenAutoMakerEnv.getI18N().t(prefix + ".nozzles"));
		headFeedsDescription.setText(OpenAutoMakerEnv.getI18N().t(prefix + ".feeds"));
		String headIconText = "-fx-background-image: url(\"/image/" + OpenAutoMakerEnv.getI18N().t(prefix + ".icon") + "\")";
		headIcon.setStyle(headIconText);
		...
		if (!typeCode.isEmpty()) {
			...
			headHoursField.setText(String.format("%d %s", (int) headData.getHourCount(), OpenAutoMakerEnv.getI18N().t("unit.hours")));
			maxTempField.setText(String.format("%d %s", (int) headData.getMaxTemp(), OpenAutoMakerEnv.getI18N().t("unit.temp")));

...
import xyz.openautomaker.root.ui.remote.RootServer;
...
public class RootStackController implements Initializable {
	...
	public void showPrinterNamePage(RootPrinter printer) {
		Platform.runLater(() -> {
			if (printerNamePage == null) {
				printerNamePage = (loadPage(namePageURL, PrinterNameController.class));
			}
	...
	public void showServerNamePage() {
		Platform.runLater(() -> {
			if (serverNamePage == null) {
				serverNamePage = (loadPage(namePageURL, ServerNameController.class));
			}
	...
	public void showCleanNozzlesMenu(RootPrinter printer) {
		Platform.runLater(() -> {
			if (cleanNozzlesMenu == null) {
				cleanNozzlesMenu = (loadPage(menuURL, CleanNozzlesMenuController.class));
			}
	...
	public void showEjectStuckMenu(RootPrinter printer) {
		Platform.runLater(() -> {
			if (ejectStuckMenu == null) {
				ejectStuckMenu = (loadPage(menuURL, EjectStuckMenuController.class));
			}
	...
	public void showIdentityMenu(RootPrinter printer) {
		Platform.runLater(() -> {
			if (identityMenu == null) {
				identityMenu = (loadPage(menuURL, IdentityMenuController.class));
			}
	...
	public void showMaintenanceMenu(RootPrinter printer) {
		Platform.runLater(() -> {
			if (maintenanceMenu == null) {
				maintenanceMenu = (loadPage(menuURL, MaintenanceMenuController.class));
			}
	...
	public void showPrintMenu(RootPrinter printer) {
		Platform.runLater(() -> {
			if (printMenu == null) {
				printMenu = (loadPage(menuURL, PrintMenuController.class));
			}
			hidePages(printMenu);
	...
	public void showSecurityMenu(RootPrinter printer) {
		Platform.runLater(() -> {
			if (securityMenu == null) {
				securityMenu = (loadPage(menuURL, SecurityMenuController.class));
			}
	...
	public void showSettingsMenu(RootPrinter printer) {
		Platform.runLater(() -> {
			if (settingsMenu == null) {
				settingsMenu = (loadPage(menuURL, SettingsMenuController.class));
			}
	...
	public void showServerSettingsMenu() {
		Platform.runLater(() -> {
			if (serverSettingsMenu == null) {
				serverSettingsMenu = (loadPage(menuURL, ServerSettingsMenuController.class));
			}
	...
	public void showTestAxisSpeedMenu(RootPrinter printer) {
		Platform.runLater(() -> {
			if (testAxisSpeedMenu == null) {
				testAxisSpeedMenu = (loadPage(menuURL, TestAxisSpeedMenuController.class));
			}

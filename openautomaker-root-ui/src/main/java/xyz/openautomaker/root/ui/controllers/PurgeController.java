...
import xyz.openautomaker.root.ui.remote.FilamentDetails;
import xyz.openautomaker.root.ui.remote.HeadEEPROMData;
import xyz.openautomaker.root.ui.remote.PurgeData;
import xyz.openautomaker.root.ui.remote.PurgeTarget;
...
public class PurgeController implements Initializable, Page {
	...
	public void initialize(URL url, ResourceBundle rb) {
		translateLabels(m1Title,
				...
				m2PurgeTempLabel,
				m2PurgeTempSuffix);
		purgePromptText = OpenAutoMakerEnv.getI18N().t(purgePromptText);
		purgeUnavailableText = OpenAutoMakerEnv.getI18N().t(purgeUnavailableText);
	...
	private void updatePurgeData(PurgeData pData) {
		Platform.runLater(() -> {
			...
			if (showPanel1) {
				...
				m1PreviousTemp = Math.round(headData.getRightNozzleLastFTemp());
				if (m1PreviousTemp > -1)
				...
				m1NewTemp = Math.round(attachedFilaments[0].getFilamentTemperature());
			...
			if (showPanel2) {
				...
				m2PreviousTemp = Math.round(headData.getLeftNozzleLastFTemp());
				if (m2PreviousTemp > -1)
				...
				m2NewTemp = Math.round(attachedFilaments[1].getFilamentTemperature());
